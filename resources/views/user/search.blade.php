    ...
    <div id="news-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($news as $item)
            <a href="{{ route('news.detail', $item['id']) }}"
                ...
                <img class="object-cover w-full h-40 rounded-t-lg" src="{{ asset('storage/' . $item->image_url) }}"
                    alt="{{ $item['title'] }}">
                <div class="flex flex-col justify-between p-4 leading-normal text-center">
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">
                        {{ \Carbon\Carbon::parse($item['date'])->format('F j, Y') }} - {{ $item->category->name }}</p>
                    <h5 class="mb-1 text-lg font-semibold tracking-tight text-gray-900 dark:text-white">{{ $item['title'] }}
                    </h5>
                </div>
            </a>
        @endforeach
